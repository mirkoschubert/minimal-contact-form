...
export default function Edit({ attributes, setAttributes, clientId }: EditProps) {
  ...
  if (error || !globalSettings) {
    return (
      <div {...blockProps}>
        ...
        >
          <p>
            <strong>{__('Error:', 'mcf')}</strong> {error || __('Settings could not be loaded.', 'mcf')}
          </p>
        </div>
      </div>
    )
  ...
  return (
    <>
      <BlockInspectorControls attributes={attributes} setAttributes={setAttributes} />
      <div {...blockProps}>
        <BlockPreview settings={effectiveSettings} clientId={clientId} />
      </div>
    </>
  )
}
